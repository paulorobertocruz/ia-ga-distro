...
class Individuo{
    ...
    public function getRecursoProximo(id:Int):Predio
    {
        /*
            Retorna predio mais proximo que tem um recurso alocado nele
        */
    ...
    public function getPredioVazioProximo(id:Int):Predio
    {
        var dist:Float = -1;
        var j:Int = -1;
        ...
        for( i in 0...GerentePredio.size()){
            ...
            if(dist == -1 && ! recursos.exists(GerentePredio.get(i).id))
            {
                dist = GerentePredio.distancia(id, i);
                j = i;
            }
            else if(dist > GerentePredio.distancia(id, i) && ! recursos.exists(GerentePredio.get(i).id))
            {
                dist = GerentePredio.distancia(id, i);
                j = i;
            }
    ...
    public function shiftRecurso(id:Int):Void
    {
        var nextPredio:Predio = getPredioVazioProximo(id);
        recursos[nextPredio.id] = recursos[id];
        recursos.remove(id);
    }
